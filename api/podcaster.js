...
function getPodcastFeedUrl(podcastId) {
	return getData({ url: `${PODCAST_ID_DATASOURCE_URL}${podcastId}` }) // ttl in minutes
		.then(data => {
...
export function getAllPodcasts() {
	return getData({ url: PODCASTS_DATASOURCE_URL }/*, { ttl: 60 * 24 }*/) // ttl in minutes
		.then(data => {
			return data.feed.entry.map(data => new PodcastSummary(data));
		});
}
	...
	return new Promise((resolve, reject) => {
		Promise.all([
			...
			.then(values => {
				...
				getData({ url: `${feedUrl}` }) // ttl in minutes
					.then(doc => {
